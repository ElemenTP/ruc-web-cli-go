...
GOBUILD=CGO_ENABLED=0 go build -trimpath -ldflags '-X "ruc-web-cli/cmd.Version=$(VERSION)" \
		-X "ruc-web-cli/cmd.BuildTime=$(BUILDTIME)" \
		-w -s -buildid='
...
freebsd-amd64:
	GOARCH=amd64 GOOS=freebsd GOAMD64=v2 $(GOBUILD) -o $(BINDIR)/$(NAME)-$@
